...
/*									*/
/* 22 May 2024 kbr							*/
/*		fixed dirflags for normal DOS				*/
/*		added SDRIVE*.COM to sort first				*/
/*									*/
/*									*/
...
/************************************************************************/
int mydirsort(const struct dirent **a,const struct dirent **b)
{
	char an[12],bn[12];
	...
	if (rootdir) {
		if (strcmp(an,"SDRIVE  COM")==0) return(-1);
		if (strcmp(bn,"SDRIVE  COM")==0) return( 1);
		if (strcmp(an,"SDRIVEN COM")==0) return(-1);
